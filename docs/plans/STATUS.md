...
| F823 | `pi-subagents` resolves the permission system by package name to inject it into each child, trying `$agentDir/npm/node_modules/<name>` then `$agentDir/extensions/<name>` (`resolvePermissionSystemExtension`). A Nix install populates neither, but the second exists anyway because `permissionSystemConfigFiles` writes `config.json` there. The resolver finds a config directory where a package belongs, demands a `package.json`, and refuses to spawn. | Every subagent run failed with "Permission-system package manifest is missing", which reads as a permission denial and is a failed path lookup. Fixed at pi-nix `93b164d` by linking the store path into the first candidate. The one-line alternative -- dropping a `package.json` beside that `config.json` -- would have loaded the permission system twice: `extensions/` is a pi discovery root and `loader.js` rule 3 loads any subdirectory whose `package.json` carries a `pi` field, so the directory is inert today only because it holds no manifest. `npm/node_modules` is not a discovery root. |
| F824 | **Predicted, not yet observed.** A subagent child gets `runtimeExtensions = [prompt-runtime, (fanout), permSystemExt]` (`pi-args.ts`) -- the permission system but NOT pi-automode. The chain owner reads the same `config.json`, whose `authorizerChain` names `pi-automode`, and `resolveConfiguredLinks` skips an unregistered name with a warning ("invariant 2 -- more prompting, never less"). Zero links means the composed chain IS the terminal, which is a prompt, and a child has no TTY. `settings.json` carries no `extensions` key, so the child inherits nothing ambiently either. | Subagents should spawn after F823 but have every non-fast-path tool call end in `confirmation_unavailable`, i.e. denied. Untested because no subagent has ever spawned successfully: the parent log shows 72 `authorizer_chain_resolved` and 0 `authorizer_chain_unregistered_link`. The check is to spawn one and grep the review log for `authorizer_chain_unregistered_link`; its presence confirms this, its absence refutes it. Fix direction is injecting pi-automode into children (a per-agent `extensions` entry or `subagentOnlyExtensions`), NOT a discovery-root install, which would double-load it in the parent for the F823 reason. |
| F825 | Two latent Nix-layout assumptions found by the same sweep, neither active. `pi-mcp-adapter`'s `npx-resolver.ts` resolves `npx`/`npm exec` binaries through an npm cache under `$HOME`, which is a tmpfs in the jail; dormant at 0 configured MCP servers. `pi-permission-system`'s `computeExtensionPaths` auto-allows read-only access to a discovered global `node_modules` root, which a store install does not have; costs a convenience allow, not correctness. | Recorded so neither is rediscovered as a mystery. The sweep covered all 15 loaded extensions for cross-package path resolution; `pi-subagents` is the only one that does it at all, and `resolvePermissionSystemExtension` was its only filesystem lookup of another package. Every other cross-extension reference (`pi-intercom:detach-request` and friends) is an event-bus channel name, not a path. |
