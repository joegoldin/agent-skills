...
Prereqs
- On this machine `tsci` is already installed by the dotfiles kicad module
  (`modules/home/_pkgs/tscircuit/`, run under bun). `tsci upgrade` does not
  apply to that install; bump the package version instead.
- Elsewhere: Node.js or Bun, then a global install:
