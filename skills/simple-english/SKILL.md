# Simple English — ASD-STE100 for technical writing
...
Write for that reader. Each sentence must survive one read.

The rules remove the usual marks of AI-generated text as a side effect: long sentences, synonym rotation, hedges, filler, and decorative clauses.

## Your task

When you write or rewrite technical text:

1. Select the mode: pragmatic or strict.
2. Classify each passage as procedural or descriptive. Every other rule depends on this.
3. Fix your vocabulary before you draft. Pick ONE verb for the check/verify/confirm concept and ONE noun for the config/settings concept. Use no other word for these concepts in the document.
4. Apply the rules from the catalog.
5. Run Vale, then the judgment checks. This step is not optional.
6. Never change code, identifiers, commands, or quoted errors. See Untouchables.

When you are asked to CHECK text instead of write it, report each violation as: rule number, the offending text, a compliant rewrite. Cite only rule numbers from this file. Do not cite rule numbers from memory. The numbering is not intuitive and models invent it.

## The linter: Vale with the `SimpleEnglish` style

The mechanical rules ship as a [Vale](https://vale.sh) style. Vale is markup-aware, so it does not read code blocks or inline code as prose.

```bash
vale-skill simple-english README.md              # descriptive text: 25-word limit
vale-skill simple-english:procedural runbook.md  # instructions: 20-word limit, condition first
vale-skill simple-english:strict docs/           # everything, escalated
vale-skill simple-english --output=JSON docs/    # machine-readable
```

Run the procedural profile on procedures and the descriptive profile on descriptions. The profiles differ because the rules differ: an instruction stops at 20 words (Rule 5.1) and a description stops at 25 (Rule 6.3).

Vale checks 16 rules: sentence and paragraph length, contractions, unapproved modals, perfect and progressive tenses, "-ing" clauses, semicolons, Latin abbreviations, trailing conditions, phrasal verbs, passive voice, telegraph style, the slop table, and synonym rotation.

Vale does not check: classification (procedural or descriptive), one instruction per sentence, one topic per paragraph, multi-word noun length, warning order, or whether a technical noun is used as a verb. Those stay yours. The catalog below is the full specification; the style is the part that survives as a rule.

If `vale-skill` is not on PATH, apply the catalog by hand and say that you worked without the linter.

## Two modes

| Mode | When | What you apply |
|---|---|---|
| **Pragmatic** (default) | Docs, READMEs, error messages. The user wants clear text. | All structural rules. Domain words stay legal ("idempotent", "webhook"). |
| **Strict** | The user names STE, ASD-STE100, or compliance. | Structural rules and full vocabulary discipline. Tell the user that full compliance needs the official dictionary, a free download at asd-ste100.org. |
...
| Vale profile | `simple-english:procedural` | `simple-english` |

Do not mix the two in one passage. A "Getting started" section is procedural. An "Architecture" section is descriptive. A note inside a procedure is descriptive: it gets the 25-word limit and no imperative.
...
The standard rejects "could" even for possibility: write "an explosion can occur", never "could occur". For "should": a requirement becomes "must"; a suggestion is stated as fact or deleted. This matters twice over for agent instructions, because models read "should" as optional.
...
Rule 8.6 matters for software text: `sqlpipe run --config sqlpipe.yaml` in backticks is quoted text and counts as one word. Long identifiers do not use up your sentence budget. Vale's word count is a plain token count, so a sentence heavy with identifiers can be flagged when it is compliant. Check the flag before you split the sentence.
...
General recommendations GR-1 to GR-8: keep the conjunction "that", be careful with "with", give pronouns clear referents, prefer "this + noun" over bare "this", avoid false friends, avoid Latin abbreviations, use inclusive language, and use the possessive apostrophe form only when you are sure it is correct.

GR-6 for software docs: "e.g." → "for example", "i.e." → "that is". Delete "etc." and name the items, or write "and more".

## Vocabulary discipline

The official dictionary (about 900 approved words and about 1,200 banned words with alternatives) is copyrighted by ASD and is not reproduced here. Its mechanics apply without it: **one word, one meaning, one part of speech.**
...
| oil | Noun only, as used in the STE examples. For the verb, the dictionary gives "lubricate". |
...
Collapse these rotations to one term each (Rules 1.11, 9.4). `SimpleEnglish.Consistency` catches the common pairs; the rest is yours.
...
The same rules, different targets. Long-form adaptations are in `references/use-cases.md`:
...
- **Runbooks** — STE's home ground. Imperative steps, conditions first, warnings before the step.
...
- **Agent instructions (system prompts, AGENTS.md, skills)** — a prompt is a procedure for a reader that cannot ask questions. One instruction per sentence, no "should", condition first.
- **Translation preparation** — STE's original job. One meaning per word plus complete grammar removes most translation ambiguity.

## Before you deliver

Run the mechanical pass, then the judgment pass.

**Mechanical** — `vale-skill simple-english:strict FILE`. Fix every alert or say why you kept it.

**Judgment** — five checks Vale cannot make:

1. **Classification.** Is each passage cleanly procedural or descriptive? Procedures in the imperative, descriptions never in the imperative.
2. **One instruction per sentence** (Rule 5.2). Two commands joined by "and" are two sentences.
3. **Multi-word nouns** (Rule 2.1). Any noun chain over three words gets broken with prepositions.
4. **Warning order** (Rules 7.2, 7.3). Command or condition first, risk second.
5. **Terminology.** Search for the words you did NOT pick in step 3 of Your task. Replace every hit.

When the user asked for STE compliance, end the report with this statement: "No tool can guarantee ASD-STE100 compliance. Final approval rests with the writer. The official standard is a free download at asd-ste100.org."
...
What changed: 40-word sentences split under 20; "you're" expanded; "check" and "confirm" collapsed to "make sure that"; every condition moved before its command; "etc." removed; code and error strings untouched.
...
For prose that needs voice, use `avoid-ai-writing` (subtractive) and `prose-craft` (additive) instead. For deciding what a document should be before you write it, use `diataxis`.
...
- `references/use-cases.md` — adaptations for error messages, runbooks, incident reports, commit messages, UI copy, and agent instructions
